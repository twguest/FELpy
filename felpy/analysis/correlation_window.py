...
def window(arr, nrows, ncols):
    ...
    h, w = arr.shape
    assert h % nrows == 0, "{} rows is not evenly divisble by {}".format(
        h, nrows)
    assert w % ncols == 0, "{} cols is not evenly divisble by {}".format(
        w, ncols)
...
def getWindows(arr, n):
    ...
    if BOOL:
        w = window(arr, int(arr.shape[0]//np.sqrt(n)),
                   int(arr.shape[1]//np.sqrt(n)))
    elif not BOOL:
...
def autocovariance2D(roi):
    ...
    for r in range(nx):
        roi[r, :] -= mu
        acov += autocorrelation(roi[r, :], roi.shape[1]) / \
            (np.amax(autocorrelation(roi[r, :], maxlag=roi.shape[1])))
...
def getFeatureSize(ii, nWindows=16, px=None, bPlot=False):
    ...
    if px is not None:
        ax = np.linspace(
            0, px*int(ii.shape[1]/(np.sqrt(nWindows))), int(ii.shape[1]/(np.sqrt(nWindows))))
    else:
...
def test():
    ...
    cors = getFeatureSize(ii=arr, nWindows=64,
                          px=wfr.get_spatial_resolution()[0], bPlot=True)
